...
from data.models import LyricsData, LyricsLine
...
def parse_plaintext(content: str) -> LyricsData:
    ...
    """
    lines = [LyricsLine(text=line.strip()) for line in content.split("\n") if line.strip()]
...
def lyrics_from_json(s: str) -> LyricsData:
    ...
    return LyricsData(
        ...
        lines=[LyricsLine(time_ms=item.get("time_ms", item.get("time")), text=item["text"]) for item in d.get("lines", [])],
    )
